...
2. 通知监听器仅处理 `com.android.mms` 的非群组摘要通知；同一通知会话在2.5秒窗口内合并更新并选择最长正文，再按“通知会话 + 消息时间”去重。若正文已存在于标准 SMS 收件箱，或15秒内刚触发过标准 SMS 广播，则忽略通知路径。后一规则用于兼容 ColorOS 将同一条5G消息同时投递为普通短信广播和压缩通知正文的情况。
