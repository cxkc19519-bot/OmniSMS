...
- 短信入口：声明式 `BroadcastReceiver` 负责进程未运行时的系统唤醒；常驻前台服务同时注册同权限保护的动态接收器，绕过 ColorOS 启动管理偶发跳过清单接收器的问题。服务还监听系统收件箱记录变化，广播被跳过时立即按递增编号补查。三条入口共用本地指纹去重。
- 5G消息入口：`NotificationListenerService`；回调第一步只允许 `com.android.mms`，群组摘要和其他包全部丢弃。
...
- 权限：运行时申请 `RECEIVE_SMS` 和 `READ_SMS`。`READ_SMS` 仅用于服务恢复后的遗漏补查；首次授权先记录短信库最新编号，不批量上传此前历史短信。首页检测电池优化豁免状态，并提供系统授权入口；ColorOS 自启动和后台运行开关仍需用户在系统设置中确认。
- 通知使用权：系统授权在技术上可访问所有通知，但实现禁止解析白名单以外通知；首次授权不处理通知栏旧消息，服务重连只恢复24小时内仍存在的系统短信通知。
- 本地队列：Room/SQLite。
...
1. 接收器从广播中合并多段短信；服务重新启动时，从持久化短信库编号游标开始补查最多24小时的系统收件箱。不能使用系统时间作为唯一游标，因为 ColorOS 可能保存短信中心时间。
2. 通知监听器仅处理 `com.android.mms` 的非群组摘要通知；同一通知会话在2.5秒窗口内合并更新并选择最长正文，再按“通知会话 + 消息时间”去重。若正文已存在于标准 SMS 收件箱，或15秒内刚触发过标准 SMS 广播，则忽略通知路径，并将该通知事件记录为已处理，避免服务重连后补发旧通知。后一规则用于兼容 ColorOS 将同一条5G消息同时投递为普通短信广播和压缩通知正文的情况。通知正文开头由 ColorOS 添加的 `[数字条]` 聚合计数在入队前移除，正文其他位置的相同文字保持不变。
3. 三条采集路径使用发送方、正文和短信源时间生成不可逆指纹，在本地唯一约束下去重。
4. App 生成不可预测的消息 ID，并将短信加密写入本地数据库；首次获得读取权限时只记录当前最大短信编号，不上传旧短信。
5. 短信广播入口在回调返回前同步完成本地加密落盘；网络上传仍由前台服务在最长30秒的部分唤醒锁内异步执行。PDU 暂不可解析或落盘失败时，前台服务5秒后补扫收件箱，WorkManager 每次重试前也先补扫。
6. 服务端验证时间戳、请求随机数和签名，拒绝重放或无效请求。
7. 服务端按 `device_id + message_id` 执行幂等插入。
8. 服务端使用发送 Gmail 的 SMTP 授权生成邮件，并投递到独立的接收邮箱。
9. 成功后返回稳定状态；App 将本地项标为已发送。
10. App 和 VPS 定期清理超过24小时的内容或状态。
...
- `last_error_code`
- `created_at_utc`
- `source_fingerprint`：仅本地保存的 SHA-256 去重值，不含可还原正文。
...
2. ColorOS 12.1 自启动、省电限制下的可靠性。**发现单独上划任务会杀死前台服务并产生恢复空窗；已增加系统收件箱补偿扫描，仍需完成清理后真机验收。**
...
| 2026-08-17 | 增加收件箱补偿扫描 | ColorOS 上划清理会连续杀死前台服务；服务恢复后补查并去重，避免空窗期短信永久遗漏。 |
| 2026-08-17 | 增加系统短信通知备用通道 | 真机确认部分验证码显示为5G消息，不产生标准 SMS 广播且不进入标准短信表；用户明确同意通知使用权。 |
| 2026-08-23 | 将短时唤醒保护前移到短信广播入口，并检测电池优化豁免 | `goAsync()` 只延长广播处理生命周期，不保证锁屏设备 CPU 持续运行；Android 12+ 又会限制后台启动前台服务，因此必须覆盖落盘空窗并明确引导系统白名单。 |
| 2026-08-24 | 前台服务增加动态短信接收器和收件箱观察器 | OPPO 锁屏真机记录显示 ColorOS 接收并放行短信后，仍将定向给清单 `SmsReceiver` 的广播标记为 `Skipped`；动态接收原始广播可避开厂商清单启动代理，收件箱变化则作为不依赖广播的即时补查信号。 |
| 2026-08-24 | 广播内同步持久化并增加延迟补查 | 真机再次确认系统广播已交付但异步落盘未形成邮件；关键短信改为在 onReceive 返回前入加密队列，失败时5秒后和 WorkManager 重试前补扫。 |
| 2026-08-24 | 通知匹配到系统短信记录时由当前入口确保入队 | 不能把“正文存在系统短信表”等同于“普通通道已发送”；通知入口改用系统记录和标准指纹落盘，避免跨通道竞态并保持单封去重。 |
| 2026-08-24 | 5G消息通知合并窗口持有短时部分唤醒锁 | 锁屏时间线证明通知按时发布，但 ScheduledExecutor 在深度休眠中冻结至解锁；最长10秒唤醒保护覆盖2.5秒正文合并和可靠入队，完成后立即释放。 |
