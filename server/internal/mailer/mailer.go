...
func (s SMTP) message(d model.Delivery, forwardedAt time.Time) string {
	...
	plain := fmt.Sprintf("OmniSMS · %s\r\n\r\n发送方：%s\r\nSIM：%s\r\n短信接收时间：%s\r\n实际转发时间：%s\r\n投递类型：%s\r\n\r\n完整短信内容：\r\n%s\r\n", deliveryType, d.Sender, sim, received, forwarded, deliveryType, d.Body)
	boundary := "omnisms_" + strings.Split(messageID, "@")[0]
	return normalizeBody(fmt.Sprintf("From: %s\nTo: %s\nSubject: %s\nDate: %s\nMessage-ID: <%s>\nMIME-Version: 1.0\nContent-Type: multipart/alternative; boundary=\"%s\"\n\n--%s\nContent-Type: text/plain; charset=UTF-8\nContent-Transfer-Encoding: 8bit\n\n%s\n--%s\nContent-Type: text/html; charset=UTF-8\nContent-Transfer-Encoding: 8bit\n\n%s\n--%s--\n", sanitizeHeader(s.From), sanitizeHeader(s.To), subject, forwardedAt.Format(time.RFC1123Z), messageID, boundary, boundary, plain, boundary, messageHTML(sender, sim, received, forwarded, deliveryType, d.Body), boundary))
}

func messageHTML(sender, sim, received, forwarded, deliveryType, body string) string {
