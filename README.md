...
| Android 候选版 | `0.3.5` / `versionCode 9` | 已完成构建、单元测试、发布版 Lint 和签名校验，并以保留数据的方式覆盖安装到真机。 |
...
`0.3.5` 针对 ColorOS 锁屏时偶发跳过清单短信接收器的问题，增加了以下保护：

- 前台服务运行期间动态注册受系统短信权限保护的接收器。
- 监听系统短信收件箱变化，广播遗漏时立即触发增量补查。
- 短信广播落盘期间持有最长 15 秒的部分唤醒锁。
- 合并密集的收件箱扫描请求，最多执行当前扫描和一次后续补扫，避免任务堆积和额外耗电。
- 首页检测电池优化豁免状态，并提供系统授权入口。

2026-08-24 的锁屏真机测试中，一条 5G消息验证码成功转发且 Gmail 只收到一封；升级后还自动补发了两条此前遗漏的普通 SMS。该结果证明锁屏投递、收件箱恢复和本轮去重有效，但普通 SMS 在锁屏状态下的“即时广播路径”仍需独立复测。
...
ColorOS 可能把上划清理视为强制结束，即使前台通知存在也可能停止实时服务。`0.3.5` 增加了动态接收和恢复补查，但为获得最高可靠性，仍建议保留前台服务通知并避免主动划掉 App。
