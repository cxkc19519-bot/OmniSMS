...
| Android 候选版 | `0.3.8` / `versionCode 12` | 已完成构建、单元测试、Lint、签名校验和保留数据覆盖安装；修复通知合并定时器在锁屏深度休眠时冻结的问题。 |
...
ColorOS 可能把上划清理视为强制结束，即使前台通知存在也可能停止实时服务。`0.3.8` 增加了同步可靠入队、动态接收、延迟恢复补查、跨通道可靠接管和通知合并短时唤醒保护，但为获得最高可靠性，仍建议保留前台服务通知并避免主动划掉 App。
