...
| Android 候选版 | `0.3.6` / `versionCode 10` | 已完成构建、单元测试、Lint、签名校验和保留数据覆盖安装；升级后自动追回本轮锁屏漏转发短信。 |
...
ColorOS 可能把上划清理视为强制结束，即使前台通知存在也可能停止实时服务。`0.3.6` 增加了同步可靠入队、动态接收和延迟恢复补查，但为获得最高可靠性，仍建议保留前台服务通知并避免主动划掉 App。
